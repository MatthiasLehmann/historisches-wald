...
const ImageEditorModal = ({ isOpen, onClose, image = null, onSaved }) => {
  ...
  useEffect(() => {
    ...
    setSaving(false);
    setCommentText('');
    const nextForm = image
      ? {
          title: image.title ?? '',
          year: image.year ? String(image.year) : '',
          ...
          fileType: image.file?.type === 'remote' ? 'remote' : 'local',
          filePath: image.file?.path ?? '',
          fileOriginalUrl: image.file?.originalUrl ?? ''
        }
      : { ...defaultForm };
    setForm(nextForm);
    setReviewerName(image?.review?.reviewer ?? '');
    setStatusValue(image?.review?.status ?? 'pending');
  }, [image, isOpen]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (form.fileType === 'local' && !payload.file.path) {
        throw new Error('Bitte geben Sie einen lokalen Pfad an.');
      }

      if (form.fileType === 'remote' && !payload.file.originalUrl) {
        throw new Error('Bitte geben Sie eine Remote-URL an.');
      }

      const saved = image?.id
        ? await updateImageAsset(image.id, payload)
        : await createImageAsset(payload);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await updateImageReviewStatus(image.id, { reviewer: reviewerName.trim(), status: statusValue });
      onSaved?.();
    } catch (statusError) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      await addImageReviewComment(image.id, {
  ...
  const handleCompleteReview = async () => {
    if (!image?.id || !reviewerName.trim()) {
      setError('Reviewer-Name erforderlich.');
      return;
    }
    try {
      await completeImageReview(image.id, { reviewer: reviewerName.trim() });
      onSaved?.();
    } catch (completeError) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center overflow-y-auto p-6">
      <div className="bg-white rounded-md shadow-xl w-full max-w-4xl">
        <header className="flex items-center justify-between border-b border-parchment-dark px-6 py-4">
          <div>
            <p className="text-xs uppercase tracking-[0.5em] text-ink/60">Bild bearbeiten</p>
            <h2 className="text-2xl font-serif font-bold">{image ? 'Metadaten aktualisieren' : 'Neues Bild anlegen'}</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <label className="space-y-1 text-sm font-medium text-ink/80">
              ...
              <input
                ...
                onChange={handleFieldChange}
                className="w-full border border-parchment-dark rounded-sm px-3 py-2"
                placeholder="z.B. rights-reserved"
              />
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <label className="space-y-1 text-sm font-medium text-ink/80">
              ...
              <input
                ...
                onChange={handleFieldChange}
                className="w-full border border-parchment-dark rounded-sm px-3 py-2"
                placeholder="/images/img-00001.jpg"
              />
            </label>
            <label className="space-y-1 text-sm font-medium text-ink/80">
              Original-URL
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="md:col-span-1 border border-parchment-dark rounded-sm p-3 bg-parchment/40">
              <p className="text-xs uppercase tracking-[0.4em] text-ink/60 mb-2">Vorschau</p>
              {previewUrl ? (
                <img src={previewUrl} alt={form.title} className="w-full h-40 object-cover rounded-sm" />
              ) : (
                <div className="w-full h-40 bg-white border border-dashed border-parchment-dark/70 flex items-center justify-center text-xs text-ink/50">
                  Keine Vorschau verfügbar
                </div>
              )}
            </div>

            <div className="md:col-span-2 border border-parchment-dark rounded-sm p-3 space-y-3 bg-parchment/10">
              <div className="flex items-center justify-between">
                ...
                {image && image.linkedDocuments?.length > 0 && (
                  <div className="text-xs text-ink/60 text-right">
                    Verknüpft mit: {image.linkedDocuments.join(', ')}
                  </div>
                )}
              </div>
